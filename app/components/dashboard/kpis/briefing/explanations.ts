...
export const Q_EXPLAIN: Record<string, string> = {
  ...
  Q8: 'Per originating project type, direct NNA plus the NNA rolled up from the follow-up chain it spawned, across all history.',
  Q9: 'For each project-type × department cell: the % of completed work that landed NNA, over the median NNA amount, across all completed history.',
  Q10: 'Projects flagged “Follow Up” (delivered, NNA pending) that started 6+ months ago, oldest first.',
  Q11: 'Flow of engagement volume from intake channel → project type → outcome; the table lists the most common end-to-end journeys. Chosen scope & period.',
  Q12: 'Per project type, the % of engagements that spawned at least one follow-up (a child in the chain); types with ≥8 occurrences, across all history.',
