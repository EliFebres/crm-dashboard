...
// PATCH /api/offices/:id — admin only. Body: { name }. Cascades the rename.
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await renameOrg('office', id, typeof body.name === 'string' ? body.name : '');
...
// DELETE /api/offices/:id — admin only. Refuses while anyone is still assigned.
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const name = await deleteOrg('office', id);
    void logActivity(req, {
