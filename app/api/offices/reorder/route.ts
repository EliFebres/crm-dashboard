...
import { reorderOrg } from '@/app/lib/db/org';
...
// PATCH /api/offices/reorder — admin only. Body: { ids: string[] }
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    await reorderOrg('office', ids);
    void logActivity(req, {
      action: 'office.reorder',
      entityType: 'office',
      details: { count: ids.length },
    });
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[PATCH /api/offices/reorder]', err);
