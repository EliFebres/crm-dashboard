...
import { reorderProjectTypes } from '@/app/lib/db/projectTypes';
import { logActivity } from '@/app/lib/activity/log';

const NO_DB = { error: 'Database not configured. Set SQLITE_DIR to enable write operations.' };

// PATCH /api/project-types/reorder — editors only. Body: { ids: string[] }
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    await reorderProjectTypes(ids);
    void logActivity(req, {
      action: 'projectType.reorder',
      entityType: 'projectType',
      details: { count: ids.length },
    });
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[PATCH /api/project-types/reorder]', err);
